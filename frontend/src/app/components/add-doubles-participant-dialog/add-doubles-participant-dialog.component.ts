...
import { Player, DoublesParticipant } from '../../../shared/supabase.types';
...
export class AddDoublesParticipantDialogComponent implements OnInit {
  ...
  async addParticipant() {
    if (
      ...
      this.error.set(
        this.translate.instant('DISCIPLINE_DETAIL.PARTICIPANTS.ADD.SELECT_BOTH_PLAYERS'),
      );
      return;
    }
    ...
    if (this.selectedPlayer1Id === this.selectedPlayer2Id) {
      this.error.set(
        this.translate.instant('DISCIPLINE_DETAIL.PARTICIPANTS.ADD.DIFFERENT_PLAYERS'),
      );
      return;
    }
    ...
    if (this.disciplineGender === 'mixed') {
      if (this.selectedPlayer1.gender === this.selectedPlayer2.gender) {
        this.error.set(
          this.translate.instant('DISCIPLINE_DETAIL.PARTICIPANTS.ADD.MIXED_GENDER_REQUIRED'),
        );
        return;
      }
    }
